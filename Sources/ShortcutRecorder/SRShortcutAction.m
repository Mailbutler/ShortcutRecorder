    ...
    switch (anEventType)
    {
        ...
        case NSEventTypeFlagsChanged:
        {
            ...
            if (keyCode == kVK_Command || keyCode == kVK_RightCommand)
                ...
                os_log(OS_LOG_DEFAULT, "#Error Unexpected key code %{public}hu for the FlagsChanged event", keyCode);
            break;
        }
        default:
...
- (OSStatus)handleEvent:(EventRef)anEvent
{
    ...
    os_activity_initiate("-[SRGlobalShortcutMonitor handleEvent:]", OS_ACTIVITY_FLAG_DETACHED, ^{
        ...
        {
            os_log_error(OS_LOG_DEFAULT, "#Critical Failed to get hot key ID: %{public}d", error);
            return;
        }
        else if (hotKeyID.id == 0 || hotKeyID.signature != SRShortcutActionSignature)
        {
            os_log_error(OS_LOG_DEFAULT, "#Error Unexpected hot key with id %{public}u and signature: %{public}u", hotKeyID.id, hotKeyID.signature);
            return;
        }
        ...
        @synchronized (self->_actions)
        {
            ...
            switch (GetEventKind(anEvent))
            {
                ...
                case kEventHotKeyReleased:
                    ...
                    break;
                default:
                    os_log(OS_LOG_DEFAULT, "#Error Unexpected key event of type %{public}u", GetEventKind(anEvent));
                    return;
            }
...
- (void)_installEventHandlerIfNeeded
{
    ...
    if (error != noErr)
    {
        os_log_error(OS_LOG_DEFAULT, "#Critical Failed to install event handler: %{public}d", error);
...
- (void)_removeEventHandlerIfNeeded
{
    ...
    if (error != noErr)
        os_log_error(OS_LOG_DEFAULT, "#Error Failed to remove event handler: %{public}d", error);
...
- (void)_registerHotKeyForShortcutIfNeeded:(SRShortcut *)aShortcut
{
    ...
    {
        os_log_error(OS_LOG_DEFAULT, "#Critical Failed to register Carbon hot key: %{public}d (keyCode: %{public}u, modifierFlags: %{public}lu)", error, aShortcut.keyCode, aShortcut.modifierFlags);
        return;
    }
...
- (void)_unregisterHotKeyForShortcutIfNeeded:(SRShortcut *)aShortcut
{
    ...
    if (error != noErr)
    {
        os_log_error(OS_LOG_DEFAULT, "#Critical Failed to unregister Carbon hot key %{public}u: %{public}d (keyCode: %{public}u, modifierFlags: %{public}lu)", hotKeyID, error, aShortcut.keyCode, aShortcut.modifierFlags);
    }
    else
    {
        if (SRDebugLogsEnabled()) {
...
CGEventRef _Nullable _SRQuartzEventHandler(CGEventTapProxy aProxy, CGEventType aType, CGEventRef anEvent, void * _Nullable aUserInfo)
{
    ...
    {
        os_log_error(OS_LOG_DEFAULT, "#Error #Developer The system disabled event tap due to %{public}u", aType);
        CGEventTapEnable(self.eventTap, true);
        return anEvent;
    }
    else if (aType != kCGEventKeyDown && aType != kCGEventKeyUp && aType != kCGEventFlagsChanged)
    {
        os_log_error(OS_LOG_DEFAULT, "#Error #Developer Unexpected event of type %{public}u", aType);
        return anEvent;
    }
    else
...
+ (NSDictionary<NSString *, id> *)_parseSystemKeyBindings
{
    ...
    if (@available(macOS 10.13, *))
    {
        ...
        {
            os_log_error(OS_LOG_DEFAULT, "#Error unable to read system key bindings (error: %{public}s)", error.localizedDescription.UTF8String);
            systemKeyBindings = @{};
        }
    }
    else
...
+ (NSDictionary<NSString *, id> *)_parseUserKeyBindings
{
    ...
    if (@available(macOS 10.13, *))
    {
        ...
        if (!userKeyBindings)
        {
            os_log_debug(OS_LOG_DEFAULT, "#Error unable to read user key bindings (error: %{public}s)", error.localizedDescription.UTF8String);
            userKeyBindings = @{};
        }
    }
    else
